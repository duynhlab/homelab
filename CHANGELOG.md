...
- **`docs/api/` observability contracts** — `observability.md` is the normative application hub (document ownership, layer responsibilities, cross-signal data policy, error ownership, worker/Temporal rules, PR checklist); `api.md § Observability` delegates instead of duplicating the signal matrix; `logs.md`, `metrics.md`, `tracing.md`, and `profiling.md` aligned on runtime log levels, conditional trace fields, authoritative business-metric emission, high-cardinality ID terminology, and profiling fail-open/degraded policy. Every as-built claim verified against `duynhlab/pkg` and the ten service repos (2026-07-29); aspirational rules split out as labelled **Target** items with a Known-gaps backlog (HTTP access-log probe filter, semconv access-log fields, logger-from-config unification, auth OTLP trace correlation, profiling shutdown convergence).
