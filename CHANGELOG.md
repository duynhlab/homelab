...
- **infra (kong)**: Edge hardening on the Kong gateway (DB-less, OSS plugins). Added two global `KongClusterPlugin`s in `kubernetes/infra/configs/kong/plugins.yaml` — **`response-transformer`** injecting security headers (`X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options`, HSTS) + stripping `Server`, and **`correlation-id`** generating `X-Request-ID` (echoed downstream) so the header the CORS plugin already exposed is real. Halved the `rate-limiting` numbers (`5/s·100/min·2500/h`) and corrected its stale "single-replica" comment — with 2 Kong replicas + `policy: local` the effective limit is ~2x and approximate (DB-less has no `cluster` policy; `redis`/Valkey intentionally not adopted for a homelab). Decided **not** to add a Kong `jwt` plugin — JWT validation stays in the services (`pkg/authmw`); recorded as **[ADR-003](docs/decisions/ADR-003-jwt-validation-in-services-not-kong.md)**. `request-transformer` skipped. Research grounded in Kong docs via context7.
