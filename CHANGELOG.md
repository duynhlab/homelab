...
- **order worker: inventory address, Worker Versioning, cutover template** (RFC-0021 P3) — `INVENTORY_GRPC_ADDR` wired and the reconciler switched on, so it does useful work immediately even pre-cutover (terminal orders are all product-path, so `GetReservation` answers NOT_FOUND, the pair reads as consistent, and the backlog settles to 0 — proving the loop and its NetworkPolicy path before any order's stock depends on it). `ORDER_STOCK_PARTICIPANT=product` and `ORDER_RECONCILER_ENABLED=true` are both spelled out at their code defaults so a flip is a one-value greppable edit. The participant is emitted through a conditional block in the **domain ResourceSet**, because the `mop` chart has no such value and an RSIP input alone would be **silently dropped**. Worker Versioning declares deployment `order-fulfillment` + Build ID = image tag (ADR-030 now records that naming). `set-current-version` ships as a **suspended CronJob** template (the inventory-backfill pattern): the command fails by default until a worker of that version has polled — verified on CLI 1.7.3 — and reconciling it from the Temporal wave would deadlock against `apps-local`, which deploys that worker. `make validate` gains a guard that all **three** copies of the Build ID agree and **fails loudly on every path that cannot compare them**, because a mismatch pins new workflows to a version no worker serves and that fails with no error, no failed activity and no alert. ADR-030 Adoption corrected **Complete → Partial**: the index claimed Complete while no versioning configuration existed.

- **Temporal chart moved to `controllers/`, retired manifests deleted** — the official chart was first added under `configs/temporal/`, co-located with the operator CRs it replaced. Wrong half of the tree: `controllers/` holds platform components installed by a chart (26 HelmReleases — Kong, Valkey, OpenBAO, Vector, Jaeger…) and `configs/` holds the CRs and configuration they consume; it also left two files named `helmrelease.yaml`, one live and one dead. Now `controllers/temporal/helmrelease.yaml` under the single `temporal-local` Kustomization — excluded from the controllers umbrella exactly as `kong-local` is, because it needs `platform-db` and the ServiceMonitor CRD ordered first (the CRD comes from `controllers-local`, not `monitoring-local`; the old comment misattributed it). Its two satellites moved to the homes their object types already had instead of a Temporal-specific Kustomization: the Web UI route joined `configs/kong/ingress-infra.yaml` with every other infra UI, and the alerts became a directory under `configs/observability/metrics/prometheusrules/`. Both had been the platform's only exceptions of their kind — one the alert catalog had to document to stay accurate. **Net: one fewer Kustomization than before, not one more.** The retired operator HelmRelease and `TemporalCluster`/`TemporalNamespace` CRs are **deleted** rather than kept commented out — git holds them and ADR-030 records that a real rollback needs a `platform-db` PITR anyway. Also added both temporal paths to `scripts/flux-validate.sh`: `controllers/temporal` is excluded from `controllers/kustomization.yaml` (it needs its own Kustomization for the apps-local dependency), so **neither the chart nor its config had ever been validated**.
