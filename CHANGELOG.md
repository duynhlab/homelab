...
- **RFC-0021 baseline e2e** (phase-0 gate §0.7) — 7 saga scenarios driven against local-stack via a local-only script (happy path, magic-amount decline, two-user TOCTOU race on the last unit, duplicate confirm, shipment-failure compensation, worker-outage convergence, capture-fail documented as un-injectable); first run archived in `RFC-0021/baseline-e2e-results.md` — 18 PASS, 0 FAIL, repo SHAs recorded. The canonical tracked gate stays the local-stack README Phase A/B/C protocol.
