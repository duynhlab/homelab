...
- **docs (observability)**: Fixed observability-doc drift uncovered by the review — corrected the microservices dashboard count from "34 panels / 5 rows" to the live **40 panels / 6 rows** (`README.md`, `grafana/{README,dashboard-reference}.md`, `runbooks/observability-deep-dive.md`); fixed a stale Sloth `v0.15.0` → `v0.16.0` diagram label; and reconciled the alerting docs, which claimed Slack/routing was "planned / single default receiver", with reality — VMAlertmanager has `slack-default`/`slack-critical` receivers wired (with a `slack_api_url` **placeholder** pending secret injection), PagerDuty still planned (`alerting/{README,slo-burn-rate-alerts,dashboard-comparison}.md`).
