...
- **docs (architecture/Temporal)**: Added `docs/architecture/temporal-order-fulfillment.md` — the spec-driven design for the platform's first **Temporal** workflow (a durable **order-fulfillment saga**: reserve stock → create shipment → confirm → notify → clear cart, with activity retries + reverse-order compensation) and the **platform-grade Temporal infrastructure** (`alexandrevilain/temporal-operator` `TemporalCluster`/`TemporalNamespace` CRDs via a `HelmRepository`+`HelmRelease`, server **1.24.2** (target 1.27.x — the published operator chart only supports server `<1.25.0`; bump once it re-publishes for v0.22.0), against a dedicated CloudNativePG `temporal-db` with `temporal` + `temporal_visibility` stores authenticated by the CNPG-generated `temporal-db-app` secret, Web UI via Kong, ServiceMonitor/dashboard/alerts, worker embedded via a `worker` subcommand; the official `temporalio/helm-charts` documented as the alternative, and the operator's lack of native auto-scaling / multi-cluster replication noted as accepted gaps). Checkout stays async (201 `pending`; saga drives `confirmed`/`failed`). Design only — implementation lands phase-by-phase (infra → contracts → providers → orchestrator → deploy → e2e).
