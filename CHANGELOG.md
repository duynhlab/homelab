...
- **docs (observability/metrics)**: Restructured the fragmented `docs/observability/metrics/` folder into a clean hub + per-layer set. `README.md` is now a **hub** (fundamentals RED/USE/SLO · monitoring stack & why · one overall architecture diagram · metrics-coverage-by-layer table + methodology matrix · doc map · ops quick-start) instead of a 700-line catch-all. Split the application detail into **`metrics-apps.md`** (RED histogram, label injection, gRPC east-west, exemplars, instrumentation, leak detection, app manifest index) and the infrastructure detail into **`metrics-infra.md`** (`up`/restarts, USE coverage, Go-vs-cAdvisor, infra manifest index). **Folded `monitoring-coverage.md`** (methodology matrix + manifest index) into the README and the two layer docs, then **removed** it. Alert/recording-rule references in the layer docs point to the authoritative `docs/observability/alerting/alert-catalog.md` (no duplicated, drift-prone manifest tables); added two-way links between the alerting docs and the metrics layer docs. Normalized the Vietnamese-mixed `promql-guide.md` and `postgresql/custom-metrics.md` to **English** (and dropped emoji headers in the PromQL guide for consistency). Added **two-way cross-links** with `docs/api/` (`grpc-internal-comms.md` §4 and `api.md` conventions ↔ `metrics-apps.md`), demoted `vmauth.md` to a `victoriametrics.md` reference, and updated the `docs/README.md` + `docs/observability/README.md` doc maps. No manifest/code changes — docs only.
