...
- **docs (system-design enrichment)**: Added the guiding principle **"every decision is a tradeoff"** to the `RFC-0000` and `ADR-0000` templates (and the rfc/adr hub Conventions) so proposals state costs, not just upside. Enriched `docs/caching/caching.md` — a Mermaid decision-aid for eviction policies (scope × signal), an explicit **tradeoff** note on the deployed `allkeys-lru`, and a new **Distributed Cache (concept & current state)** section (consistent hashing / replication / hot keys / write strategies, honestly framed against the single-node Valkey we actually run + the scale-up path). Added a concise **"What is an API gateway?"** concept + tradeoffs section to `docs/platform/kong-gateway.md` (bridging to the existing pass-through/ADR-003 design). Fixed the `docs/README.md` index, which was missing both `caching/` and `kong-gateway.md`. Docs only; concepts written in-house (no external links added).

- **docs/manifests (consistency)**: Post-refactor consistency sweep (3 review agents over the whole repo). Fixed stale **Loki** references left from the v0.94.0 Loki removal — comments/echo strings in `vector.yaml`, `victorialogs/helmrelease.yaml`, `kong/helmrelease.yaml`, `openbao/helmrelease.yaml`, `openbao-bootstrap/configmap.yaml`, and stale READMEs (`controllers/logging/victorialogs/README.md` dual-ship diagram, the `auth-db`/`supporting-shared-db` cluster READMEs, `clusters/local/README.md`) → all now say VictoriaLogs (sole backend). Normalized the leftover **`55m`** ESO-refresh mentions in the secrets docs/diagrams to **`1h`** (the deployed `refreshInterval`), removing the TTL-vs-interval contradiction. Labeled the `secrets-management.md` Production Roadmap as **planned**. **Removed the stale one-off `docs/observability/observability-review.md`** (its findings are addressed/obsolete — e.g. it still listed the already-deleted `controllers/logging/loki/*` dead files) and repointed its two inbound references. Comment/doc-only — no manifest behaviour change. (Note: the `DS_LOKI` dashboard input names are cosmetic and already mapped to the VictoriaLogs datasource — left as-is.)

- **docs (secrets/proposals)**: Retired two now-superseded files from `docs/secrets/`. **`production-plan.md` → `docs/proposals/rfc/RFC-0008/implementation.md`** (moved intact via `git mv` as RFC-0008's long-form working plan — feature selection, architecture, DB-credential redesign + SQL templates, install phases, day-2 ops), so the plan lives with the RFC instead of duplicating it. **Deleted `vault.md`** (archived legacy Vault dev-mode docs, superseded by ADR-005); its Vault→OpenBAO comparison/tradeoff is preserved in the `README.md` §1 table (now the canonical comparison, cross-linked to ADR-005 + RFC-0008). Repointed all inbound links and fixed a stale `backlog.md` entry left in the `docs/README.md` tree by the previous migration.

- **docs (secrets/proposals)**: Migrated `docs/secrets/backlog.md` into `docs/proposals/` and **removed** it. Completed work → ADRs: **ADR-004** (enable OpenBAO audit logging) and **ADR-005** (run OpenBAO HA on Raft instead of Vault dev) — written fresh in OpenBAO terms, honest about the carried local-only gaps. Open items → RFC **backlog** rows (split bootstrap, secret rotation — largely superseded by dynamic creds, PushSecret). Repointed the inbound `backlog.md` links (`docs/README.md`, `docs/secrets/README.md`, `secrets-management.md`) to the proposals hub. The stale Vault-era examples in the old backlog were dropped, not carried over.

- **docs (secrets)**: Corrected `docs/secrets/` to match the deployed reality (verified with secrets-scanner + security-audit + manifest cross-check). Added a **current-state-vs-planned** banner to `README.md` and marked the database-engine sections (§5.2, §6) as **planned, not deployed** — the bootstrap enables only KV v2 + Kubernetes auth + best-effort audit, so dynamic DB creds / OIDC / KMS auto-unseal / TLS are targets, not live. Flagged that `cart`/`order` owners still use hardcoded `postInitSQL` passwords (ESO-bypassed) and that committed `*-K1nd-2026!` dev creds are local-only. Qualified the audit claim (best-effort, `auditStorage` off) and noted ESO refresh is `1h` (not the documented 55m). Added `Last updated` footers to all five active docs. Docs only; hardening tracked separately.

- **docs (API/logging)**: Removed the stale **References** section from `docs/api/logs.md` — its two `specs/active/context-aware-logging/*` links were dead (no such files in the repo); the observability cross-link is already provided by the in-Overview pointer.
...
- **docs (platform/Gitflow)**: Standardized the **3-tier environment vocabulary to `dev` / `uat` / `prod`** across `docs/platform/`. Fixed a leftover `staging` in `check_template.yml` (`dev/staging/main` → `dev/uat/main`) missed in the tier rename, and normalized the prod env/namespace **identifier** in `gitflow.md` (`production namespace` → `prod namespace`, the deploy-flow nodes, and the verification-matrix tier label) so it matches `cicd.md`'s `environment: prod` / `dev/uat/prod` GitHub Environments. Prose adjectives (`production-ready`, `production-grade`) and the `Production Gate` ruleset name are left as-is — they don't name an environment. Live config (the `local` Kind cluster, OpenBAO `secret/data/local/…` paths) is unchanged: the project still runs only on `local`.

- **docs (platform/CI-CD)**: Refreshed the CI/CD docs for the new **GoReleaser binary-release** pipeline, fixed stale claims, and **consolidated `cicd-standard.md` into `cicd.md`** (one CI/CD doc). `cicd.md`: the `v*`-tag flow is now documented as a **GoReleaser binary release** (was wrongly described as "retag the Docker digest, no rebuild"); added a detailed **Binary Releases (GoReleaser)** section (artifact contract — tarball/`.sha256`/`build-info.env` — config locations, and how it feeds `packages`); added `release.yml`/`goreleaser.yml` to the workflow split + shared-workflow list; corrected the `build.yml` trigger to **`main`-only** (+ a note distinguishing the Gitflow `dev/uat` target from currently-wired triggers); removed the contradictory migration-image node; fixed a broken link. **Merged the org standard/policy** (principles, action-pinning, permissions matrix, image-security, required-checks, adoption checklist, follow-ups) in as a **CI/CD Standard & Policy** section (deduping the catalog + scan-gate to internal anchors); **deleted `cicd-standard.md`** and repointed `docs/README.md`. `sonarcloud.md`: fixed the project-key example to the per-repo `duynhlab_<repo>` pattern and documented the **≥80% new-code coverage** Quality Gate + its exclusions.
