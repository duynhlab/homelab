...
- [x] **P1** — PR #789 (`fix/kind-preflight-blockers`) is **merged**. Without it
...
- [x] **P2** — The nine `feat/shared-http-middleware` service PRs are merged,
...
- [x] **P3** — homelab #788 is merged (docs + the portal's Compose block).

---

## K0 — The machine

- [x] **K0.1** Docker is installed **rootful**. `scripts/kind-up.sh` calls
...
- [x] **K0.2** Bring-up tools present: `kind`, `kubectl`, `flux`, `tofu` (≥1.11
  — or export `TF_BIN=terraform`), `git` with an `origin` remote.
  `make prereqs`
  **FAIL:** any `MISS` line.

- [x] **K0.3** Validation tools present. **`make prereqs` does not check these**
...
- [x] **K0.4** Host ports 80, 443 and 5050 are free. 80/443 are the kind
...
- [x] **K0.5** Egress reaches Docker Hub, `ghcr.io`, the OpenTofu registry and
...
- [x] **K0.6** Hostnames resolve: `sudo scripts/setup-hosts.sh`, then
...
- [x] **K1.1** `make validate` passes on the checked-out revision, before any
  cluster exists. It needs no cluster.
  **FAIL:** any schema or kustomize error.

- [x] **K1.2** `make up` completes. It is
...
- [x] **K1.3** The cluster is the expected shape: 1 control-plane + 3 workers on
...
- [x] **K1.4** **Every** Flux Kustomization reports Ready. 22 are declared under
...
- [x] **K2.1** Read the pins out of Git. Fill the table below at audit time —
...
- [x] **K2.2** Read the images actually running, and compare them to K2.1 line
  by line.
  ```bash
...
- [x] **K2.3** The coupled pins hold. These are not derivable from a single
...
- [x] **K2.4** `auth` is absent — **10** services, not 11. Its cluster surface
...
- [x] **K2.5** All seven ResourceSets that `apps-local` health-checks are Ready:
...
- [x] **K2.6** Every HelmRelease is Ready.
  `flux get helmreleases -A | grep -v True || echo "all Ready"`

- [x] **K2.7** *(build-arg contract — cannot be checked with kubectl)* The
...
- [x] **K3.2** Those three exceptions have not expired. All carry
...
- [x] **K3.3** OpenBAO self-unsealed and ESO is serving. No manual unseal step
...
- [x] **K4.4** Both realms exist and are the ones from Git.
  ```bash
...
- [x] **K4.5** A customer token mints through the realm. There is **no password
...
- [x] **K4.8** The realm fence holds at the edge. A **customer** token on a
...
- [x] **K5.4 Metrics — THE ROW THIS AUDIT EXISTS FOR.**
...
- [x] **K5.6 Profiles.** Pyroscope carries all 10 services; `auth` is absent.
  ```bash
...
- [x] **K5.8 Alert rules loaded, none firing wrongly.**
...
- [x] **K6.1** Fill this in and paste it into the pull request.

```markdown
## Kind E2E audit — 2026-08-17

Cluster: kind `homelab`, 4 nodes, `kindest/node:v1.34.3`
Revision: audited on `main` + the six fixes below (all merged: #791 #792 #793 #794 #795 #796)
Preconditions: #789 merged · service tags pinned (#796) · #788 merged

| Group | Rows | Result | Evidence |
|---|---|---|---|
| K0 machine | 6 | PASS | 4 tools + 3 validators present; ports free; 4/4 hostnames resolve |
| K1 bring-up | 6 | PASS (K1.6 not run) | `make up` 2m56s exit 0; **23/23 Kustomizations Ready**, 0 pods failing |
| K2 delivery | 7 | PASS | image↔pin table 15/15 exact; `auth` absent; 7/7 ResourceSets Ready |
| K3 admission/secrets | 5 | 2 PASS · 1 owner-decision · 2 script-stale | OpenBAO self-unsealed 3/3, 0 ExternalSecrets unsynced |
| K4 edge/identity | 8 | 5 PASS · 1 deviation · 2 not run | 301 → 200; issuer `CN = homelab-ca`; both realms; real JWT |
| K5 signals | 8 | 4 PASS · 2 partial · 2 not run | **K5.4 verbatim output below** |
| K6 wrap | 4 | K6.1 this block · K6.3 PASS | `make down` removes cluster **and** registry |

**K5.4 verbatim output** — `go_goroutine_count{service_name="order"}`:

    order  pod=order-689dc6ff7-l5rb5  ns=order        (1 series)

and, per `count by (service_name) (go_goroutine_count)`:

    cart 1 · checkout 1 · checkout-worker 1 · inventory 1 · mockpay 1
    notification 1 · order 1 · order-worker 1 · payment 1 · product 1
    review 1 · shipping 1 · user 1

**Verdict: there is no identity collision on the cluster** — and not because
`k8s.pod.name` disambiguates (it is present, and promoted), but because
`service.name` already differs: the workers publish as `order-worker` and
`checkout-worker`. The row's premise is false for the cluster. The collision is
Compose-only, rooted in two lines of `compose.yaml` that gave each worker its
service's `OTEL_SERVICE_NAME`. `docs/api/metrics.md` is therefore correct for the
cluster, and the fix the row proposed — `service.instance.id` in `obsx` plus
adding it to VM's `promoteResourceAttributes` — is **not** needed. Fixed in #794.

Findings: 8 defects, all filed — #791 (CRD delivery impossible via Helm; edge
unreachable from host), #792 (13/13 BackendTrafficPolicy invalid → 500 on the
entire API), #793 (OpenBAO bootstrap hangs on a standby node → whole Flux chain
wedges), #794 (keycloak-token.sh cannot reach an HTTPS Keycloak, and its failure
reads as a passing test; worker telemetry identity), #795 (PSS restricted
unsatisfiable → disabled with conditions recorded; PgDog had no probes), #796
(ADR-038 pins split out of this branch). Plus 10 documentation-drift items, see
the comment on this PR.

Decision: **BLOCKED at first run, ELIGIBLE after the six fixes.** Every defect
was fixed and re-proved on a cluster rebuilt from zero. Rows not run — K1.6,
K4.6, K4.7, K5.1, K5.3, K5.7 and part of K5.5 — need seeded data and a browser;
they are outstanding, not passed.
```
...
- [x] **K6.3** `make down` — deletes the cluster **and** the registry container,
