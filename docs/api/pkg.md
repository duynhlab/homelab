...
| **Newest tags** (2026-09-25) | `v0.47.0` for `obsx` (span privacy exporter since `v0.46.0`; `v0.45.0` RFC-0031 Task 1.1c-B: zap bridge removed, `ForceFlush` added; `v0.39.1` is a mis-tag identical to `v0.39.0`, never pin it) · `v0.44.0` for `temporalx` (slog `WithLogger`, lifecycle and workflow events, SDK error shape as `exception.message`, registry-generated constants) · `v0.37.2` for `authmw idempotency proto` · `v0.37.1` for `httpx` · `v0.37.0` for `grpcx` (slog access interceptor) · `v0.36.3` for `dbx` · `v0.36.2` for `migratex` · `v0.36.1` for `flagx` · `v0.3.0` for `logger/slogx` (`exception.message`, generated constants) and `v0.3.0` for `httpmw` (`Transport`/`Handler`, the sanctioned otelhttp hop), each on its own line from `v0.1.0` | — |
...
> `policy-lint` input) — since 2026-09-25 it runs **blocking** in all ten services and
> encodes the **version floor** with `gomodguard_v2` (a telemetry module more than one
> minor behind its current release fails the lint; the floor moves with each release,
> see pkg README § Releasing). (b) The **Weaver registry** ([ADR-076](../proposals/adr/ADR-076-semantic-convention-registry/),
> RFC-0031 Task 4.5, shipped 2026-09-24/25): `semconv/registry/` declares every platform
> name, the `slogx`/`temporalx` key and event constants are generated from it, the
> event-catalog table in [logs.md](./logs.md) is held to it in CI, and `weaver registry
> live-check` is row C22 of the compose gate.
...
|-----|---------|------|-----------------|
| `v0.3.0` | `httpmw` | 2026-09-25 | `Transport(base)` and `Handler(h, operation)` wrap otelhttp for a plain net/http client or handler, so no service links contrib instrumentation itself (the fleet lint policy is blocking from this day). |
| `v0.47.0` | `obsx` | 2026-09-24 | `db.connection_string` joins the span privacy deny list. |
| `v0.3.0` + `v0.44.0` | `logger/slogx` `temporalx` | 2026-09-24 | `Err` writes `exception.message` instead of the deprecated `error.message` (temporalx reshapes the SDK's `Error` the same way); key and event constants generated from the registry (`semconv/`, ADR-076) replace the hand-written strings. |
| `v0.46.0` | `obsx` | 2026-09-24 | Span privacy at the source: an exporter wrapper under the batcher drops `client.address`, `network.peer.address`/`port` and `user_agent.original` from every span, covering a factory-built provider too. |
