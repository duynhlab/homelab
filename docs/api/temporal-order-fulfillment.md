...
- **Retired operator** — the alexandrevilain operator HelmRelease and its `TemporalCluster`/`TemporalNamespace` CRs are **deleted** (git is the record; a genuine rollback needs a `platform-db` PITR regardless — ADR-030). The CRDs and the cert-manager admission webhook are gone with them. The chart lives under `controllers/` because that is where platform components installed by a chart live, applied by a single `temporal-local` Kustomization (excluded from the controllers umbrella like `kong-local`, since it needs a database and the ServiceMonitor CRD ordered first). Its Web UI route is an entry in `configs/kong/ingress-infra.yaml` and its alerts a directory under `configs/observability/metrics/prometheusrules/` — the homes those object types already had.
- **Worker Versioning** — the order worker declares Worker Deployment **`order-fulfillment`** (name deliberately equal to the task queue — one deployment serves one queue here) with its **Build ID = the image tag** (`1.7.0`): `TEMPORAL_WORKER_DEPLOYMENT_NAME` + `TEMPORAL_WORKER_BUILD_ID` in `kubernetes/apps/order-worker.yaml`. Both or neither — `pkg/temporalx` exits rather than start a worker that silently ignores versioning. Which version is **Current** (the one new workflows start on) is server-side state, set by hand from a **suspended CronJob** template (`controllers/temporal/worker-set-current-version-cronjob.yaml`, never scheduled): making a version Current is an operator decision mid-cutover, not a desired state to re-assert, and the command fails until a worker of that version has polled — so reconciling it from the Temporal wave would deadlock against the `apps-local` wave that deploys that worker. `make validate` enforces that **three** copies of the Build ID agree — `image.tag`, the worker env, and the CronJob's `--build-id` — because a drift pins new workflows to a version no worker serves and that fails silently: no error, no failed activity, no alert. **Configured but not yet exercised:** no versioned worker has polled a cluster and no version has been made Current. Procedure: [RFC-0021 cutover-rollback.md](../proposals/rfc/RFC-0021/cutover-rollback.md#worker-versioning--the-other-half-of-the-same-cutover).
...
- **Edge & alerts** — Kong ingress `temporal.duynh.me` (`configs/kong/ingress-infra.yaml`); `TemporalServerDown` + service/persistence error-rate `PrometheusRule`s (`configs/observability/metrics/prometheusrules/temporal/alerts.yaml`).
