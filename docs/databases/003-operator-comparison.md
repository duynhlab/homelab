...
This project runs **4 PostgreSQL clusters** managed by two different operators: **1 CloudNativePG primary cluster (`cnpg-db`) + 1 CloudNativePG DR replica cluster (`cnpg-db-replica`)**, plus **2 Zalando clusters** (`auth-db`, `supporting-shared-db`). Both operators are production-grade, but they follow fundamentally different architectural philosophies.
...
|---------|----------|-------------------|
| **cnpg-db** | CloudNativePG | Primary CNPG cluster: PostgreSQL 18; `product`, `cart`, and `order` databases; PgDog pooler; declarative extensions; Barman backups (see backup doc) |
| **cnpg-db-replica** | CloudNativePG | DR replica cluster; separate object-store prefix under the same CNPG bucket; topology per `Cluster` manifest |
...
|---------|-----------|---------------|-----|-----|-----|
| cnpg-db | CNPG Instance Manager | CNPG Operator | Kubernetes API | ~10-30s | Per replication config in cluster spec (in-cluster replicas) |
| cnpg-db-replica | CNPG Instance Manager | CNPG Operator | Kubernetes API | ~10-30s | DR standby (follows primary) |
...
- **[Database Guide](./002-database-integration.md)** -- Main database integration guide (clusters, poolers, secrets)
- **[Backup Strategy](./006-backup-strategy.md)** -- Barman (CNPG) vs WAL-G (Zalando) backup architecture
- **[Replication Strategy](./004-replication-strategy.md)** -- Sync vs async replication per cluster
- **[Connection Poolers](./008-pooler.md)** -- PgBouncer (Zalando), PgDog (`cnpg-db`); PgCat documented for comparison only (not used for CNPG here)
- **[Extensions](./009-extensions.md)** -- Extension management per operator
