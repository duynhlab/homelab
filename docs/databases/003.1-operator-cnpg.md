...
- The plugin-backed restore path is proven for `product-db`: PITR drill
  [DR-2026-08-A](./010.2-restore-and-failover-drills.md#dr-2026-08-a--drill-a-product-db-pitr-the-barman-acceptance-gate)
  (2026-08-07) restored and validated a throwaway cluster in 2m12s.
  `platform-db` restore and DR promotion remain unproven — run those drills
  before declaring the migration production-ready for every dataset.
...
- Barman plugin restore is drill-recorded for `product-db` only (DR-2026-08-A,
  2026-08-07); `platform-db` restore and DR promotion still lack recorded
  evidence, so measured production readiness is per-dataset, not platform-wide.
...
| Backup integration | Barman Cloud Plugin + `ObjectStore` CRs | Plugin-backed PITR validated for `product-db` (DR-2026-08-A) |
| Restore evidence | One recorded drill (`product-db` PITR, 2026-08-07, 2m12s); `platform-db` + DR promotion unproven; not yet recurring | Run and record `platform-db` restore + promotion drills; make drills recurring |
...
---
_Last updated: 2026-08-31 — recorded the product-db PITR drill (DR-2026-08-A, 2m12s); platform-db and DR-promotion evidence gaps kept explicit._
