...
The current manifests use in-tree `barmanObjectStore`:
...
- DR replica bootstrap through `externalClusters[].barmanObjectStore`.

### Production-forward direction

CNPG upstream has moved backup integration toward the Barman Cloud Plugin and
CNPG-I `ObjectStore` resources. The concepts do not change: base backup, WAL
archive, PITR, and restore remain the same. The implementation changes from
in-tree backup fields to a plugin reference.
...
- New production deployments should prefer Barman Cloud Plugin + `ObjectStore`.
- Existing deployments should migrate only after a restore drill proves the new
  path works.
- Do not delete old backup prefixes during migration.
- Keep rollback to in-tree `barmanObjectStore` available until plugin-backed
...
- The Barman plugin migration is an additional lifecycle item for current
  deployments using in-tree `barmanObjectStore`.
...
| Backup integration | In-tree `barmanObjectStore` | Plan Barman Cloud Plugin migration |
