...
| `auth-db` | CloudNativePG | `auth` | 3-node HA cluster (sync quorum `ANY 1`) for auth |
| `shared-db` | CloudNativePG | `user` | Single-node shared cluster for user, notification, shipping, review |
...
flowchart TB
  ...
  subgraph otherClusters ["Other CloudNativePG clusters"]
    authDb["auth-db 3 nodes HA (sync ANY 1)"]
    sharedDb["shared-db 1 node"]
    temporalDb["temporal-db 1 node (no backup)"]
  end
...
| `auth-db` | CNPG cluster healthy, 3/3 ready |
| `shared-db` | CNPG cluster healthy, 1/1 ready |
...
## CNPG DRP: `auth-db` and `shared-db`

### Current homelab state

`auth-db` is a 3-node CloudNativePG cluster with synchronous quorum `ANY 1` and
a `pgdog-auth` pooler — the same HA posture as `product-db`. Its DR path is the
Barman Cloud Plugin backup at `s3://pg-backups-cnpg/auth-db/` (restore-to-new-cluster).

`shared-db` is a single-instance CloudNativePG cluster (SPOF) with the
`pgdog-shared` pooler. It has Barman Cloud Plugin backups at
`s3://pg-backups-cnpg/shared-db/` but no automatic database failover; recovery
is restore-to-new-cluster or scaling `spec.instances` to add HA replicas.

> These clusters were migrated from the Zalando operator to CloudNativePG. The
> historical Zalando/Patroni scaling runbook
> ([zalando-ha-scaling.md](./runbooks/zalando-ha-scaling.md)) is kept for
> reference only.

### Production baseline

- Set `spec.instances: 3` for workloads that require HA (as `auth-db` and `product-db` already do).
- Keep synchronous quorum for critical writes; decide per data class whether relaxed durability is acceptable.
- Use Barman Cloud Plugin backups with recorded restore drills.
- Use a CNPG replica cluster (`spec.replica.enabled: true`) when a separate DR
  cluster is required, as `product-db-replica` demonstrates.
...
| `auth-db` primary pod crash | CNPG HA failover | 0 for commits acknowledged by `ANY 1` quorum | Seconds to under a minute | Estimated |
| `shared-db` pod/storage loss | Barman restore-to-new-cluster (single instance, no failover) | Since last backup/WAL archive | Manual restore time | Requires drill |
...
- `shared-db` remains a single-node SPOF.
...
- Decide whether `shared-db` should be scaled to 3 instances (CNPG HA) to remove the SPOF.
...
_Last updated: 2026-07-11 — All clusters now on CloudNativePG (Zalando retired); auth-db/shared-db DRP reframed to CNPG with Barman Cloud Plugin backups._
