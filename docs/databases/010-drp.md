...
The platform runs five PostgreSQL clusters:

| Cluster | Operator | Namespace | Role |
|---------|----------|-----------|------|
| `cnpg-db` | CloudNativePG | `product` | Primary CNPG cluster for `product`, `cart`, `order`, `payment` |
...
| `temporal-db` | CloudNativePG | `temporal` | Single-node backing store for the Temporal server — **no backups / no WAL archiving yet**, so it has **no DR coverage** |
...
- `temporal-db` is a single-node cluster with **no backups and no WAL archiving** — a total loss of `temporal-db` loses all in-flight Temporal workflow state; no PITR or DR path exists for it yet.
...
- Add backups + WAL archiving (and scale past a single instance) for `temporal-db`.
