...
cluster on one host**. `cnpg-db-replica` is even in the same `product` namespace
as `cnpg-db`.
...
flowchart TB
  subgraph host ["Single host / single Kind cluster"]
    subgraph productNs ["Namespace product"]
      primary["cnpg-db (3 instances)"]
      dr["cnpg-db-replica (1 instance)"]
    end
...
The mechanism, however, is already DR-ready: `cnpg-db-replica` bootstraps and
follows purely from the object store (`bootstrap.recovery` +
`replica.enabled: true` against the `cnpg-db-primary` external cluster). Nothing
...
1. **Stage 1:** add zone anti-affinity + topology spread to `cnpg-db` and move
   `cnpg-db-replica` to a different zone/node.
...
3. **Stage 2:** stand up a second-region bucket with replication, then a second
   cluster hosting `cnpg-db-replica` that recovers from it.
