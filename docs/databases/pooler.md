...
This document provides a detailed analysis of the connection pooling strategies used in the platform, including architecture, trade-offs, and configuration details for **PgBouncer**, **PgCat**, and **PgDog**.
...
| **Used In** | `auth-db`, `supporting-db` | `transaction-shared-db` | `product-db` |
...
**Used by**: `transaction-shared-db` (`cart`, `order` services)

**Architecture:**
- **Standalone Deployment**: Runs as a separate Deployment (`pgcat-transaction`) in `cart` namespace.
...
- **Function**: Connection pooling + potential for advanced routing.
...
| **High Read Traffic** | **PgCat** | Automatic routing of SELECTs to replicas saves Primary resources. |
| **Sharding** | **PgCat** | Built-in sharding logic. |
| **Extreme Concurrency** | **PgCat / PgDog** | Multi-threaded architecture scales better on multi-core nodes. |
