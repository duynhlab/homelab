...
The Zalando operator creates the databases listed under `spec.databases` **only
during a successful first-time init**. On a slow spilo cold boot the operator's
DB-connection retry window can expire before PostgreSQL is listening, leaving the
cluster in `PostgresClusterStatus: CreateFailed`; subsequent syncs bring up
Patroni and roles but never (re)create the databases, so application migrations
fail with `database "…" does not exist`. To make this deterministic, each Zalando
cluster dir ships an idempotent `ensure-databases` Job (`auth-db` → `auth`;
`supporting-shared-db` → `user`/`notification`/`shipping`/`review`) that connects
as the superuser and runs guarded `CREATE DATABASE … OWNER …` (Postgres has no
`IF NOT EXISTS` for databases). It runs in the `databases-local` wave and
self-heals on re-run. Keep the Job's (database → owner) list in sync with
`spec.databases`. See
`kubernetes/infra/configs/databases/clusters/auth-db/ensure-databases-job.yaml`
and
`kubernetes/infra/configs/databases/clusters/supporting-shared-db/ensure-databases-job.yaml`.
