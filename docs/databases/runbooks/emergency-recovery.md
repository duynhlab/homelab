...
kubectl cnpg status product-db -n product            # if the plugin is installed
...
flowchart TB
  ...
  clusterLost["Whole product-db cluster\nlost?"]
...
```bash
kubectl cnpg status product-db -n product            # new primary should appear, 3/3 ready
```

If it does not self-heal, force a planned switchover to a healthy replica:

```bash
# `switchover` is NOT a plugin command — `promote` is, and it names the instance.
...
## Scenario 2 — whole `product-db` cluster lost, DR replica healthy

**Symptom:** the primary cluster is gone/frozen but `product-db-replica` is alive and
...
bootstrap a fresh cluster directly from `s3://pg-backups-cnpg/product-db/` (latest
...
the primary is back (re-apply `product-db-replica`).
