...
kubectl get deployment auth -n auth -o yaml | grep OTEL_COLLECTOR_ENDPOINT

# Check collector config
kubectl get configmap -n monitoring -l app.kubernetes.io/name=opentelemetry-collector -o yaml
```
...
- Test with debug exporter enabled
...
2. Update collector to export only to Tempo
3. Monitor for issues
4. Decommission Jaeger

**If Choosing Jaeger:**
1. Configure persistent storage (Badger/Cassandra/ES)
...
- Route production services to Tempo
- Route development to Jaeger
- Monitor both systems

**Phase 3: Cutover**
- Migrate all services to chosen backend
- Keep other backend for validation
- Decommission when confident

## Best Practices

### Instrumentation

1. **Use consistent naming**: Service names, operation names
2. **Add meaningful attributes**: User ID, request ID, business context
3. **Record errors properly**: Use `RecordError()` for exceptions
4. **Set span status**: OK, Error, Unset
5. **Filter noise**: Skip health checks, metrics endpoints

### Sampling

1. **Start conservative**: 10% sampling in production
2. **Sample errors higher**: Errors should be sampled at 100%
3. **Adaptive sampling**: Adjust based on service health
4. **Head vs tail**: Use head-based for volume, tail-based for errors

### Resource Management

1. **Batch exports**: Reduce network overhead
2. **Memory limits**: Prevent OOM in collector
3. **Timeout configuration**: Balance latency vs reliability
4. **Monitor metrics**: Track export success rate, latency
