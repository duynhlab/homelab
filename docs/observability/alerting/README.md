...
flowchart TD
    ...
    subgraph destinations ["6. Alert Destinations"]
        ...
        Slack["Slack receivers<br/>configured · webhook pending"]
    ...
    class PR,PSL,VMRule data;
    class Sloth,VMOp,VMAlert,VMAM,Karma,Grafana platform;
    class Infra,Slack external;
    class PagerDuty planned;
```

### Current State

- Stages 1-4 are fully operational (163 static alerts, 64 Sloth SLO burn-rate alerts — see the [alert catalog](alert-catalog.md)).
...
flowchart TD
    ...
    subgraph layer2 ["Layer 2: SLO Burn-Rate Alerts"]
        PSL["PrometheusServiceLevel CRDs<br/>10 services x 3 HTTP SLOs<br/>+ inventory x 2 gRPC SLOs"]
        Sloth["Sloth Operator<br/>generates multi-window burn-rate rules"]
        T2["64 SLO alerts<br/>page + ticket severity"]
    end
...
**Coverage**: 10 HTTP services x 3 SLOs + inventory x 2 gRPC SLOs = 32 SLOs, 64 alerts
...
| Layer 2: SLO alerts | 64 alerts (32 SLOs x 2 severities) | Implemented |
...
- [Microservices runbooks](../runbooks/microservices/README.md) — per-alert investigation (50 files) + investigation workflows, threshold tuning, retired-alert context
