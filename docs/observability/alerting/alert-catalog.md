...
`prometheusrules/**` only, so it misses the **10 Temporal rules** (9 names) that live in
...
**230** — 222 here, minus the 2 gated CNPG topology rules that never deploy, plus
those 10 (the three ADR-055 capacity rules landed 2026-09-05). The Sloth-generated 62 are separate by design and are not in this
number.
...
| [Temporal / Pyroscope / Watchdog](#8-temporal--pyroscope--watchdog) | 14 | Tracing, workflows, worker capacity (ADR-055), profiling, dead-man's-switch, OTLP collector |
...
| TemporalScheduleToStartLatencyHigh | warning / critical | p99 `temporal_workflow_task_schedule_to_start_latency_seconds` >0.2s (warning) / >1s (critical), by `task_queue` | Workers under-provisioned — tasks wait for a poller before any error fires; the KEDA scaler (ADR-055) should already be reacting | 10m / 5m | [TemporalScheduleToStartLatencyHigh](../runbooks/temporal/TemporalScheduleToStartLatencyHigh.md) |
| TemporalTaskQueueBacklogGrowing | warning | `max by (taskqueue) (approximate_backlog_count)` >10 — server metric, label `taskqueue` | Queue depth twice the scaler's `targetQueueSize`: scaler at its ceiling, not rendered for that version, or failing its poll | 10m | [TemporalTaskQueueBacklogGrowing](../runbooks/temporal/TemporalTaskQueueBacklogGrowing.md) |
...
them hides its own saturation. `TemporalScheduleToStartLatencyHigh` is a warning/critical pair
under one name (the Sloth page/ticket precedent), so the group holds 10 rules for 9 names. 2026-08-18 correction: `TemporalServiceErrorRateHigh` had been written against
...
| ClickHouseDiskAlmostFull | warning | `DiskFreeBytes / DiskTotalBytes < 0.15` | MergeTree refuses writes near full; the 90-day TTL can't outrun sustained ingest. On local-path this ratio is the NODE filesystem, not the PVC — see the note below | 15m | [ClickHouseDiskAlmostFull](../runbooks/clickhouse/ClickHouseDiskAlmostFull.md) |
| ClickHouseDiskCritical | critical | same ratio `< 0.05` | Write failures imminent. Growing the PVC is not an option on local-path; drop partitions or free the node | 5m | [ClickHouseDiskCritical](../runbooks/clickhouse/ClickHouseDiskCritical.md) |
