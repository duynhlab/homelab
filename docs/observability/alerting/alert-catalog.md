...
| [Envoy Gateway edge](#2-envoy-gateway-edge) | 16 | The single API ingress for the whole platform — proxy fleet + control plane + the cert it actually serves |
...
| DBClientErrorRate 💤 *`db_client_operation_errors_total` is not emitted — the other 12 `db_client_*` metrics are* | warning | `rate(db_client_operation_errors_total)>0.1/s` | Sustained DB operation failures (SQLSTATE, timeouts, broken conns) — requests are failing | 5m | [DBClientErrorRate](../runbooks/microservices/DBClientErrorRate.md) |
...
| EdgeUpstreamNoHealthyEndpoints | critical | per-`envoy_cluster_name`: `membership_healthy == 0` and `membership_total > 0` | The end state EdgeUpstreamUnhealthy warns about — the route answers 503/UH on every request while the backend's own metrics go quiet, not red. `total > 0` excludes a Service with no endpoints (a config state, 503/NC). VERIFY-AT-KIND | 2m | [EdgeUpstreamNoHealthyEndpoints](../runbooks/envoy-gateway/EdgeUpstreamNoHealthyEndpoints.md) |
| EdgeUpstreamTimeoutRatioHigh | warning | per-`envoy_cluster_name`: `upstream_rq_timeout / upstream_rq_completed` >5%, with a >0.05 req/s traffic guard | One route's backend crossing the 15s BTP `requestTimeout` — 504/UT on that route while the edge-wide 5xx ratio never moves. The checkout CPU-limit incident in metric form. VERIFY-AT-KIND: the timeout counter must exist per cluster at rest | 10m | [EdgeUpstreamTimeoutRatioHigh](../runbooks/envoy-gateway/EdgeUpstreamTimeoutRatioHigh.md) |
| EdgeCertExpiringSoon | warning | `min(envoy_server_days_until_first_cert_expiring)` <7 | The cert the fleet is **serving**, not the one cert-manager issued: a renewed Secret that never reached the proxies over SDS leaves the Certificate Ready and the listener on the old cert. 90-day cert, renewBefore 15d, so <7 means a week of broken renewal or push. VERIFY-AT-KIND | 1h | [EdgeCertExpiry](../runbooks/envoy-gateway/EdgeCertExpiry.md) |
| EdgeCertExpired | critical | same gauge <1 (it floors at 0 once expired — the upstream `< 0` form can never fire) | Every HTTPS client fails the handshake: SPA, API on :443, the ADR-062 hairpin | 15m | [EdgeCertExpiry](../runbooks/envoy-gateway/EdgeCertExpiry.md) |

**Audited against awesome-prometheus-alerts (2026-09-08).** The community Envoy
set (20 rules) was mapped rule by rule: 5 were already covered here with
better shapes (the 5xx pair, membership degraded, health-check failures, outlier
ejections), 4 were adopted above, and 11 were declined with the reason recorded
in the rule file — heap memory duplicates `KubePodMemoryNearLimit`, connection
overflow counters stay at 0 because no connection limit is configured, circuit
breakers are not set in any `BackendTrafficPolicy`, generic 4xx ratios are
client noise at an API edge (the 401 and 429 rules above are the actionable
4xx), `server_live` flaps on every rolling restart, and listener SSL errors are
scanner noise. Upstream has no latency, no rate-limit, no auth and no
control-plane rule at all; those eight stay homelab-only.
...
| CNPGClusterStandbyNotStreaming | critical | `cnpg_pg_replication_in_recovery == 1 and on(namespace,pod) cnpg_pg_replication_is_wal_receiver_up == 0` | A standby has stopped replicating — not slow, disconnected. CNPG still reports it `healthy`, and this is the only rule that covers the **DR cluster**, which the chart selector excluded | 5m | [CNPGClusterStandbyNotStreaming](../runbooks/postgresql/CNPGClusterStandbyNotStreaming.md) |
...
| Alert | Sev | Metric & trigger | Impact | for | Runbook |
|-------|-----|------------------|--------|-----|---------|
| KubeDeploymentReplicasMismatch | warning | spec ≠ ready replicas — **expression repaired 2026-09-06**: it named `kube_deployment_status_ready_replicas`, which KSM does not publish (the real name transposes the last two words), so it matched nothing and could never fire | Service degraded; can't scale | 15m | [KubeDeploymentReplicasMismatch](../runbooks/kubernetes/KubeDeploymentReplicasMismatch.md) |
...
| KubeJobFailed | warning | `kube_job_status_failed>0`; command uses `exported_namespace` | Batch/backup task failed | 5m | [KubeJobFailed](../runbooks/kubernetes/KubeJobFailed.md) |
...
| TemporalPersistenceErrorRateHigh | warning | `persistence_error_with_type`/`persistence_requests` >2% — **numerator narrowed 2026-09-06**: `serviceerror_*NotFound` is Temporal control flow (does this execution exist? does this queue have user data?), and counting it fired the alert at 0.02592 against a 0.02 threshold while the real error rate was 0.0 | Workflow state not persisting — data risk | 10m | [TemporalPersistenceErrorRateHigh](../runbooks/temporal/TemporalPersistenceErrorRateHigh.md) |
...
**Audited against awesome-prometheus-alerts (2026-09-08).** The community
ClickHouse set (23 rules) was mapped rule by rule against this group. Five were
already covered (disk pair, readonly replica, ZooKeeper exceptions, delayed
inserts, node reachability via the exporter). **Seven were adopted**, all on the
per-pod `:9363` scrape with a `job="clickhouse-server"` selector and a
`VERIFY-AT-KIND` marker, because none has had the two-form pass yet:
`ClickHouseServerNotScraped` (the `absent()` guard the group lacked — ten rules
read only that scrape and would go silent together), `ClickHouseTooManyPartsPerPartition`
(the dimension the insert guards actually enforce; `ClickHouseTooManyParts` is
the server-wide trend), `ClickHouseInsertsRejected` and `ClickHouseInsertsFailing`
(the follow-up recorded on 2026-08-28, restored from the compose twin on the
ProfileEvents the server publishes at zero), `ClickHouseReplicationLag`
(`ReplicasMaxAbsoluteDelay` — the one replicated-topology signal nothing here
had), `ClickHouseKeeperSessionLost` (the gauge between the exceptions rate and
the readonly end state, and the one a Keeper restart actually exercises) and
`ClickHouseReplicatedDataLoss` (critical at the first increment). Eleven were
declined with the reason in the rule file: no `Distributed` tables and no
`insert_quorum`, so `NO_AVAILABLE_REPLICA`, `TOO_FEW_LIVE_REPLICAS` and the
Distributed-inserts rule cannot apply; cgroup memory duplicates
`KubePodMemoryNearLimit`; no `backups` disk; interserver, TCP and network-rate
thresholds are capacity noise at three replicas. The upstream set's own
weaknesses are worth knowing before copying from it again: several criticals
use `for: 0m`, nothing filters by `job` (and `ClickHouseMetrics_*` overlaps the
Keeper scrape here), and its node-down rule is a bare `up == 0` that cannot see
a scrape with zero targets.
...
`DBClientErrorRate` joined this list on 2026-09-06. It is not a naming mistake:
twelve `db_client_*` metrics exist on the cluster, including
`db_client_operation_duration_seconds_*`, so the RFC-0017 W4 instrumentation is
live — it simply never emits an **error** counter. Until a service does, the rule
is dormant, and it was listed as active until this audit.
