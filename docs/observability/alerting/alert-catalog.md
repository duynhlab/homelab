...
| TemporalTaskQueueBacklogGrowing | warning | `sum by (taskqueue, task_type, worker_version) (approximate_backlog_count)` >10 — server metric, label `taskqueue`. Summed over `partition` **only**: that is what `DescribeTaskQueueEnhanced` aggregates, and it is the number KEDA scales on | Queue depth twice the scaler's `targetQueueSize`: scaler at its ceiling, not rendered for that version, or failing its poll | 10m | [TemporalTaskQueueBacklogGrowing](../runbooks/temporal/TemporalTaskQueueBacklogGrowing.md) |
