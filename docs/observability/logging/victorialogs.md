...
| **Edge runtime** | `namespace`, `service` (= proxy pod name), `pod_name`, `container_name` | Envoy *process* lines via the dedicated Vector source (ADR-061). The edge **access log is not in this store** — it is ClickHouse-only; see [hub § edge](README.md#edge-logs-adr-061-access--clickhouse-runtime--victorialogs) |
