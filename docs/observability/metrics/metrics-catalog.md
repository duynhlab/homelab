...
| `order_fulfillment_start_outbox_failed` | `order.fulfillment.start_outbox.failed` · Observable gauge | — | Rows that hit the attempt cap. **Terminal — nothing retries them**, so any non-zero value is a human worklist, not a transient |
