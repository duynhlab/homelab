...
| **Metrics** | `etcd_disk_backend_commit_duration_seconds_bucket` |
...
Backend commit p99 above threshold — etcd's storage backend is slow to commit.

**It cannot fire here.** This platform does not scrape etcd, so `etcd_disk_backend_commit_duration_seconds_bucket`
