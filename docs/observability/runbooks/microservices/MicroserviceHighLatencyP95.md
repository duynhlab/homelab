...
2. Find the slow request in VictoriaLogs (filter on high latency), then open its `trace_id` in VictoriaTraces (traces<->logs correlation). Exemplars are not available -- VictoriaMetrics does not support them (RFC-0014 D-14)
