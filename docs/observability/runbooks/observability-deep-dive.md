...
| **Latency** | `MicroserviceHighLatencyP95` (>1s), `MicroserviceHighLatencyP99` (>2s), `MicroserviceLatencyCritical` (P95>2s) | warning (all three — the page is the Sloth latency burn-rate) | RED: Duration |
...
**Result**: Alert-to-root-cause path reduced from hours to minutes. 4-pillar correlation means a metric spike leads to the correlated logs (by `app` + `trace_id`), then to the offending trace, then to the flamegraph showing the bottleneck. 32 SLOs across all 11 services with automated error-budget tracking. MTTR improved by ~40%.
...
- 32 SLOs total (3 per HTTP service x 10, plus 2 gRPC SLOs for inventory); all but inventory's auto-generated by the chart
