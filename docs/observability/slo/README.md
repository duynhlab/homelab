...
flowchart TD
    ...
    PR -->|evaluate| Prom["Prometheus"]
    Prom -->|query| Grafana["Grafana Dashboards"]
    Prom -->|fire| AM["Alertmanager"]

    SM["ServiceMonitor<br/>component: api"] -->|scrape| Prom
```
...
4. Prometheus evaluates rules, tracks error budgets, fires alerts
5. ServiceMonitor auto-discovers services via `component: api` label
...
Sloth Operator generates recording and alerting rules for each SLO:
