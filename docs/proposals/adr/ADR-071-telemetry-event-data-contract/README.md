...
| **Adoption** | Complete — deny list enforced at the source for logs (`slogx`) and spans (`obsx` v0.46.0+ exporter wrapper) with the collector as backstop; the typed fields the 2026-09-24 audit found (`to`, origin/destination, `user.id`) are gone at the source |
...
| Strip the deny list from spans the instrumentation libraries write | `duynhlab/pkg` (source) · Observability (backstop) | RFC-0031 Final gate (2026-09-24 finding) | **Backstop done:** the collector's `transform/privacy` deletes `client.address`, `network.peer.*`, `user_agent.original` and `user.id` from every span (and the edge's `client`/`user_agent` from logs) — 0 on Kind after it. **Source pending:** `user.id` out of four services' spans and a deny-list filter in `obsx` |
...
| 2026-09-24 | Accepted / Partial | **Where the deny list is enforced for spans.** The slogx facade applies it to logs at the source, but spans were never covered: otelgin and otelhttp put `client.address`, `network.peer.address`/`port` and `user_agent.original` on every HTTP server span (9 services), four services set `user.id` (review-class) on spans, and the edge writes `peer.address`, `user_agent` and a `client` IP — all stored in ClickHouse for 90 days. Owner decision: the collector strips them for every producer now, and remains the backstop for producers the shared package never sees (the edge, Keycloak); the shared package removes them at the source in a later release. The rule itself is unchanged |
| 2026-09-25 | Accepted / Complete | **Complete.** Enforcement at the source on both signals plus the collector backstop; the compose gate's privacy rows read 0 deny keys on app spans (control `http.route` 494), 0 legacy keys on stdout, and the error shape is `error.type` + `exception.message` fleet-wide. |

---
_Last updated: 2026-09-25 — Adoption `Complete` at the close of RFC-0031 (both final gates passed). Previously span enforcement: the collector strips the deny list from every producer's spans (backstop); source-side stripping follows. Previously 2026-09-23 — Task 0.2 frozen: catalog, access severity, field classification and schema owners in logs.md. Previously 2026-09-23 — the privacy boundary ships in `logger/slogx` v0.1.0, keys and values, before both sinks. Previously 2026-09-17._
