...
---
_Last updated: 2026-09-18 — ADR-072 Adoption `Partial` — the pin and linter convergence obligation closed 2026-09-18 (RFC-0031 Phase 1 prerequisite). Previously 2026-09-17 — ADR-070 through ADR-076 created at `Accepted` with RFC-0031 at Adoption `Not started`: one shared slog facade with a reviewed event catalog, the canonical event/access/privacy data contract, a one-release cutover with no migration mechanism, the metrics, profiling and tracing contracts, and a Weaver semantic-convention registry that keeps the platform's bare namespaces as registered exceptions. Nothing is installed; Phase 0 of the RFC-0031 delivery plan is eligible. Previously 2026-09-09 — ADR-066 through ADR-069 accepted with RFC-0030 at Adoption `Not started`: PeerDB transport, source egress, `pg_cron` freshness heartbeats, and the read-only analytics boundary. Mandatory Phase 0 qualification blocks rollout and no component is installed. Previously 2026-09-05 — ADR-055 **Accepted** and installed (KEDA 2.20.2 wave, `ScaledObject` allow-list, one `WorkerResourceTemplate` per worker, two capacity alerts); Adoption **Partial** until the Kind audit runs. Previously 2026-08-28 — ADR-065 created at `Accepted` with [RFC-0028](../rfc/RFC-0028/) (ClickHouse 1×3 on a Keeper quorum) and **amended the same day**: schema ownership moved from the collector's exporter to a bootstrap Job in a `Replicated` database, after the exporter-owned path was measured leaving replicas without a schema. Adoption `Complete` on the same PR's Kind gate. Previously: ADR-062's index row caught up with the record itself (Accepted/Complete since 2026-08-26; the flux-web third client landed 2026-08-27). Previously: ADR-056 accepted (k6 assertion layer; ADR-045 sizing amended alongside it, and ADR-055 finally has an observed backlog to scale on)_
