...
| [RFC-0028](RFC-0028/) | ClickHouse replication (optional least-privilege hardening; research reaches sharding/Distributed — explicitly not built) | infra | researching | research 2026-08-28; owner-numbered same day. Rungs: schema ownership (two verified options) → CHK + 1×3 ReplicatedMergeTree; 3-user model demoted to an optional side-rung (owner call); sharding stays a documented trigger table, not a deliverable |
