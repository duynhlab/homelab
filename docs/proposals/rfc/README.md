...
| **Trace/log backend consolidation** — five trace sinks down to two (VictoriaTraces + ClickHouse); resolves [ADR-040](../adr/ADR-040-tempo-community-helm-chart/) (`Proposed`, never decided) and the [ADR-032](../adr/ADR-032-tempo-operator-monolithic/) → ADR-040 lineage. Hard prerequisite: move span-metrics off Tempo's metrics-generator onto the collector's `span_metrics` connector (already proven in local-stack) | platform-wide | [RFC-0014](RFC-0014/), [RFC-0019](RFC-0019/) |
...
---
_Last updated: 2026-08-23 — backlog gains **Trace/log backend consolidation** (five trace sinks → two, resolving the ADR-032 → ADR-040 lineage); no RFC number reserved yet. 2026-08-21 — RFC-0026 **Accepted**: research gate passed and the Temporal Worker Controller adopted, retiring the per-build manifest and the hand-run activation Job; ADR-054 and ADR-055 created at `Proposed` (KEDA recorded, not installed). Previously: RFC-0025 `implemented` (storefront cutover as frontend v3.0.0, ADR-052 Accepted / Adoption Complete) and RFC-0023 Accepted (Backoffice portal + first `protected` APIs; ADR-047..049 created at Accepted)._
