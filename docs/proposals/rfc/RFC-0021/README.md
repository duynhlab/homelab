...
| Workflow migration | Versioned branch so open histories replay the Product path; mechanism resolved in phase 3: **Worker Deployment Versioning** ([ADR-030](../../adr/ADR-030-temporal-workflow-versioning/), live 2026-07-30 — workflows run `Pinned`, one worker manifest per build, activation via CronJob template) |
...
- Phase-0 baseline was verified against local-stack via the [Phase A/B/C release audit](../../../../local-stack/docs/e2e-audit.md) — the canonical, tracked e2e gate.
