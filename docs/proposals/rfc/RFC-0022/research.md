...
| 9 | Kong rotation runbook | ~~Two-step rotation: add the new realm key at higher priority → update Kong's declarative credential in the same change~~ **Superseded 2026-08-10 by [RFC-0024](../RFC-0024/README.md)**: the edge moves to Envoy Gateway with `remoteJWKS` auto-refresh — no edge rotation step exists; only the realm-side key procedure remains. The edge check itself is kept (ADR-006 principle, re-homed by ADR-044). |
