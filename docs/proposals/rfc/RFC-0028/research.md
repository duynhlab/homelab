...
| System tables | **corrected 2026-09-04** — not "no TTLs". Audited on the deployed cluster: `query_log`/`part_log`/`trace_log` carry a 30 d TTL from an Altinity operator config override (which also re-partitions them daily and drops `query_thread_log`), and three more inherit one upstream. The real gap is five tables — `metric_log`, `asynchronous_metric_log`, `text_log`, `error_log`, `background_schedule_pool_log` — with no TTL at all. "On the 10Gi PVC" was also wrong: `standard` is `rancher.io/local-path`, so the request is advisory and growth lands on the node filesystem | short DELETE TTLs **plus a matching `PARTITION BY`** (those five are monthly-partitioned) and a drop of the `*_0` tables ClickHouse leaves behind when a system table's engine changes — see [Partitions and TTL](../../../observability/clickhouse/fundamentals.md#the-alignment-rule) |
...
_Last updated: 2026-09-04 — the **System tables** row in "vs platform as-built" was corrected against the deployed cluster: it claimed no TTLs and unbounded growth "on the 10Gi PVC", when six tables do carry a 30-day TTL (three of them from an Altinity operator override that also re-partitions them daily), the untouched set is five tables, and local-path makes the 10Gi request advisory. The remediation column now names the partition-alignment and `*_0`-leftover constraints that make the quick win less quick than it reads. Earlier 2026-08-28 — gate PASSED: Option B chosen, Q4 closed (keep default access), checklist ticked, README.md authored. Earlier same day: owner resolved open questions 1/2/3/5 (recreate; straight to 3+3; default replica path; no remote_servers action). Same day: user model demoted to optional; research opened._
