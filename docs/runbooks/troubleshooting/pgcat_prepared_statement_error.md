...
for svc in cart order product auth user notification review shipping; do
  cd ~/Working/duynhlab/${svc}-service && go mod tidy && cd - >/dev/null
done
