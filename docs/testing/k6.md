...
flowchart TB
    ...
    subgraph "Middleware Pipeline (FILTERED)"
        ...
        Prometheus["Prometheus Middleware<br/>(/metrics Prometheus-compatible)"]
        Logging[Logging Middleware]
    end
    
    subgraph "Observability Stack (CLEAN)"
        Tempo["Tempo (business spans)"]
        PromDB["VMSingle (business metrics)"]
    ...
    subgraph "Infrastructure Monitoring (INDEPENDENT)"
        ...
        PromScraper["VMAgent via ServiceMonitor (still works)"]
    end
...
- ✅ Prometheus middleware filters infrastructure endpoints (exposes `/metrics` for Prometheus-compatible scrape) — defense in depth
...
- **Docker Image**: `ghcr.io/duynhlab/k6:scenarios` (built from `k6/Dockerfile`)
...
- **Docker image**: `ghcr.io/duynhlab/k6` (built and published from the k6 code repository/pipeline)
...
- **Why this matters**:
  ...
  - Storage efficiency (~75% reduction in stored metrics time series)
...
- Health checks: Removed from load tests (infrastructure monitoring handled separately)
  - Kubernetes probes and metrics scraping (VMAgent) handle health and RED monitoring
...
- Full-stack performance analysis in VictoriaMetrics/Grafana
...
3. **VMAgent** → Scrapes `/metrics` (typically 15–30s interval; preserves tags) into **VMSingle**
...
Environment variables are configured in `kubernetes/apps/k6.yaml` (applied via Flux HelmRelease):
...
1. **Update Helm values:**
   ```bash
   # Edit kubernetes/apps/k6.yaml (values are inline in HelmRelease)
   ```
