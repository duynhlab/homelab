...
| Grafana | http://localhost:3002 | 18 dashboards in three folders: Observability (RED, business, Temporal incl. server row, OTel Collector health, Keycloak — Identity, Inventory Service — Stock Authority, Order Saga & Payment — Cutover Baseline), Gateway (3 vendored Envoy boards + Edge Overview), ClickHouse (6, incl. **Server / Engine**); Explore over VictoriaMetrics, VictoriaLogs, VictoriaTraces, ClickHouse, and Pyroscope |
