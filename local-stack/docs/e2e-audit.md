...
for d in microservices-otel-local business-otel-local temporal-worker-local red-spanmetrics \
         ...
         clickhouse-server-engine \
         heHhNSFf6Na8vIZWRs8H 8WkEOMnANKE6PW5hhpVv bdn8lriao7myoa eg-edge-sre; do
...
| C18 | Dashboard inventory | `/api/search?type=dash-db` returns exactly the 17 provisioned uids (incl. the three vendored Envoy Gateway dashboards plus the hand-authored edge SRE board under Gateway/, the collector-health board, and the two RFC-0021-era parity copies) and each loads via `/api/dashboards/uid/…` with 200 |
