...
# Build init images first (needed before app images)
...
for SERVICE in "${INIT_SERVICES[@]}"; do
    ...
    BUILD_CMD="docker build -t $INIT_IMAGE"
    if [ "$NO_CACHE" = "true" ]; then
        BUILD_CMD="$BUILD_CMD --no-cache"
        echo "   Building with --no-cache flag"
    fi
    BUILD_CMD="$BUILD_CMD services/migrations/$SERVICE/"
    
    # Build with retry mechanism
    for attempt in 1 2 3; do
        if eval $BUILD_CMD; then
            echo "$INIT_IMAGE built successfully"
            break
        else
            echo "Build attempt $attempt failed, retrying..."
            if [ $attempt -eq 3 ]; then
                echo "Failed to build $INIT_IMAGE after 3 attempts"
                exit 1
            fi
        fi
    done
    ...
    for attempt in 1 2 3; do
        if kind load docker-image $INIT_IMAGE --name monitoring-local; then
            echo "$INIT_IMAGE loaded to Kind"
            break
        else
            echo "Load attempt $attempt failed, retrying..."
            if [ $attempt -eq 3 ]; then
                echo "Failed to load $INIT_IMAGE to Kind after 3 attempts"
                exit 1
            fi
        fi
    done
    
    # Push to registry if not using local Kind
    if [ "$REGISTRY" != "localhost:5001" ]; then
        docker push "$INIT_IMAGE" || echo "Failed to push $INIT_IMAGE (may need docker login)"
    fi
done
...
for SERVICE in "${SERVICES[@]}"; do
    ...
    if [ "$FORCE" != "true" ] && [ "$NO_CACHE" != "true" ] && check_image_in_kind "$REGISTRY/$SERVICE"; then
        echo "$FULL_IMAGE already exists in Kind, skipping build"
        echo "   Use --no-cache or --force to rebuild anyway"
        continue
    fi
...
docker build --build-arg SCRIPT_NAME=load-test-multiple-scenarios \
  ...
  -t $REGISTRY/k6:v5 \
  k6/
kind load docker-image $REGISTRY/k6:v5 --name monitoring-local
