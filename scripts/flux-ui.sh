...
# Envoy Gateway edge (HTTP + HTTPS). The proxy Service name is generated by
# Envoy Gateway (envoy-<ns>-<gateway>-<hash>), so resolve it by the
# owning-gateway label instead of hardcoding it.
echo "Starting Envoy Gateway port forward (8000 HTTP, 8443 HTTPS)..."
EDGE_SVC=$(kubectl -n envoy-gateway get svc \
  -l gateway.envoyproxy.io/owning-gateway-name=platform -o name | head -1)
if [ -n "$EDGE_SVC" ]; then
  kubectl port-forward -n envoy-gateway "$EDGE_SVC" 8000:80 8443:443 > /dev/null 2>&1 &
else
  echo "WARN: no Envoy Gateway proxy Service found (Gateway 'platform' not ready?)"
fi

# Frontend (direct access, bypassing the gateway)
...
echo "Envoy Gateway (HTTP):  http://gateway.duynh.me:8000"
echo "Envoy Gateway (HTTPS): https://gateway.duynh.me:8443"
