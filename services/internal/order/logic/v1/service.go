...
func (s *OrderService) GetOrder(ctx context.Context, id string) (*domain.Order, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("invalid order id %q: %w", id, ErrOrderNotFound)
	}

	// Query order
	query := `SELECT id, user_id, total_amount, status FROM orders WHERE id = $1`
	var userID int
	var totalAmount float64
	var status string

	err = db.QueryRowContext(ctx, query, orderID).Scan(&orderID, &userID, &totalAmount, &status)
	if err != nil {
